...
-- SELECT name, price FROM shows AND SELECT MIN(price) FROM shows WHERE id NOT IN (SELECT id FROM shows WHERE price = (SELECT MIN(price) FROM shows));
...
-- SELECT name FROM users INNER JOIN shows ON COUNT(shows_users.id) WHERE shows.id = shows_users.show_id;
